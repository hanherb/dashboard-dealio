      ...
      <d-col lg="12" class="mx-auto mt-4">
        <d-card class="card-small edit-user-details mb-4">
          ...
          <d-card-footer class="border-top">
            <d-button size="sm" @click="editAudience($route.params.id)" class="btn-accent ml-auto d-table mr-3">
              Submit
            </d-button>
          </d-card-footer>
...
export default {
  ...
  methods: {
    getOneAudience() {
      ...
      this.axios.get(address + ":3000/get-one-audience", {params: {id: id}, headers: headers.headers}).then((response) => {
        this.input.name = response.data[0].name;
        this.input.city = response.data[0].city;
        this.input.gender = response.data[0].gender;
        this.input.age_start = response.data[0].age_start;
        this.input.age_end = response.data[0].age_end;
      });
    },
    editAudience() {
      let postObj = {
      ...
      this.axios.post(address + ':3000/edit-audience', postObj, headers)
      .then((response) => {
